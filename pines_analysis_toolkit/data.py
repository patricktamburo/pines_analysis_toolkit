...
def reduce(short_name, upload=False, delete_raw=False, delete_reduced=False, sftp='', manual_flat_path='', manual_dark_path='', manual_bpm_path='', linearity_correction=False, force_output_path=''):
    ...
    for i in pbar(range(np.size(raw_files))):
        target_filename = reduced_path / \
            (raw_files[i].name.split('.fits')[0]+'_red.fits')
        ...
        if manual_flat_path == '':
            master_flat, master_flat_name = master_flat_chooser(
                flats_path, header)
        else:
        ...
        if linearity_correction:
            log_line_coeffs_path = pines_path / \
                ('Calibrations/Linearity/log_line_coeffs.fits')
    ...
    if upload:
        ...
        files_to_upload = np.array(
            natsorted(np.array([x for x in reduced_path.glob('*.fits')])))
