...
#Input parameters
def reference_lightcurve(target_name,run_directory,aperture_radius,ref_to_analyze):
    ...
    use_refs = '' #'all' for all refs, 'choice' to specify, '' to optimize
    bin_lc = 1
    plots = 1
    ...
    #Pathing
    path = 'P:\\Photometry\\PINES\\'+run_directory+'\\'+target_name+'\\aper_phot\\photometry\\'+target_name+'_ap='+aperture_radius+'_photometry.p'

    #Read in output. 
    restored_output = pickle.load(open(path,'rb'))

    #First, remove the target from the output data. We don't want to use it to decorrelate the reference lightcurves. 
    restored_output['Flux'] = np.delete(restored_output['Flux'], 0, axis=0)
    restored_output['X position'] = np.delete(restored_output['X position'], 0, axis=0)
    restored_output['Y position'] = np.delete(restored_output['Y position'], 0, axis=0)
    restored_output['X subframe position'] = np.delete(restored_output['X subframe position'], 0, axis=0)
    restored_output['Y subframe position'] = np.delete(restored_output['Y subframe position'], 0, axis=0)
    restored_output['Background'] = np.delete(restored_output['Background'], 0, axis=0)

    #Since we usually count references starting from 1 (target is 0), and since we just removed the target from the arrays, 
    #have to subtract 1 off of ref_to_analyze to grab the right reference. 
    ref_to_analyze = ref_to_analyze - 1

    #Throw out flagged images. 
    times = restored_output['Time']

    targ_flux = restored_output['Flux'][ref_to_analyze,:]
    ...
    targ_background = restored_output['Background'][ref_to_analyze]
    ...
    use_locs_2 = np.where( (airmass < 2.5) & (targ_background < 6000) & (targ_background > 200) & (times > 2458884.3) & (targ_flux > (med-3*std)) )[0] 
    #use_locs_2 = np.where( (airmass < 2.5) & (targ_background < 6000) & (targ_background > 200) & (targ_flux > (med-5*std)))[0] 
    ...
    for i in range(len(restored_output['Flux'])):
        if i == ref_to_analyze:
            ax.plot(times,targ_flux,label='Ref. '+str(i+1)+' (target)',lw=3,marker='o')
        else:
            ax.plot(times,restored_output['Flux'][i,:][use_locs_2],label='Ref. '+str(i+1),marker='.',alpha=0.6)
    ax.legend()
    plt.show()

    #Get all potential combinations of reference stars.
    num_refs = np.shape(restored_output['Flux'][1:,:])[0] #Count the number of references (excluding the "target" reference.)
    ref_numbers = np.arange(0,num_refs+1,1)
    ref_numbers = np.delete(ref_numbers, ref_to_analyze, axis = 0) #Remove the ref_to_analyze from the ref_combinations
    ref_combinations = []
    ...
    if use_refs == 'choice':
        ref_combinations = [[2,3,4,5,6,7,8,9,10],[2,3,4,5,6,7,8,9,10]]
    ...
    for i in range(len(ref_combinations)):
        ...
        std_plot =  np.std(norm_targ_flux_decorrelated)
        ...
        if std_plot < best_stdev:
            print(std_plot)
            print('New best set found: ',ref_set, ', with standard deviation of ',np.round(std_plot,3))
            best_set = ref_set
    ...
    if plots:
        ...
        plt.savefig('P:\\Photometry\\PINES\\'+run_directory+'\\'+target_name+'\\analysis\\'+target_name+'_diagnostics_plot.png')
        
        save_dict = {'Times':times_plot,'Flux':best_norm_targ_flux_decorrelated,'Bin starts':bin_starts,'Bin ends':bin_ends}
        pickle.dump(save_dict,open('P:\\Photometry\\PINES\\'+run_directory+'\\'+target_name+'\\analysis\\'+target_name+'_lightcurve.p','wb'))
        plt.show()
        pdb.set_trace()
